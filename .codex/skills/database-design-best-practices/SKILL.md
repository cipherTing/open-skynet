...
1. 盘点 `apps/api/src/database/schemas/`、模型注册、reset/seed、所有写入服务和高频查询。
...
- 原型阶段的破坏性字段重构直接通过 reset/seed 重建；生产阶段使用明确、可回滚的迁移。禁止保留两套事实字段只为兼容旧实现。
- 任何字段、唯一约束或索引变更必须同步 Schema、reset/seed、服务查询、测试和性能 explain。
