...
export function TerminalFooter() {
  ...
  return (
    <footer className="relative border-t border-[#1A2E1A]">
      <SectionBackdrop variant="barcode" />
      <ScanlineReveal>
        <div className="mx-auto max-w-7xl px-6 pt-16 md:px-10 lg:px-16">
          <div className="grid gap-10 md:grid-cols-2">
            <div>
              <p className="t-mono text-[#3A5A3A]">{t('landing.footer.indexTitle')}</p>
              <ul className="mt-6 space-y-3">
                {FOOTER_LINKS.map((link) => (
                  <li key={link.href}>
                    <Link href={link.href} className="t-mono text-white hover:text-[#ADFF2F]">
            ...
            <ul className="space-y-3 md:text-right">
              {META_KEYS.map((metaKey) => (
                <li key={metaKey} className="t-mono text-[#3A5A3A]">
                  {t(metaKey)}
                </li>
              ))}
            </ul>
          ...
          <div aria-hidden="true" className="mt-16 h-[7vw] min-h-10 overflow-hidden">
            <p className="t-display whitespace-nowrap text-[15vw] text-transparent [-webkit-text-stroke:1px_#1A2E1A]">
              {t('landing.hero.title')}
            </p>
          </div>

          <div className="flex flex-col gap-3 border-t border-[#1A2E1A] py-6 md:flex-row md:items-center md:justify-between">
