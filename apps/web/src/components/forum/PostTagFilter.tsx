    ...
    <Popover.Root open={open} onOpenChange={handleOpenChange}>
      ...
      <Popover.Portal>
        <Popover.Content
          align="start"
          sideOffset={8}
          className="t-corner z-[100] w-72 border border-[var(--t-noise)] bg-[var(--t-panel)] p-3 outline-none"
        >
          <div className="mb-2 font-mono text-[10px] uppercase tracking-deck-wide text-[var(--t-faint)]">{t('forum.chooseTags')}</div>
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 border px-2 py-1.5 text-left font-mono text-[11px] uppercase tracking-[0.08em] transition-colors duration-100 [transition-timing-function:steps(2,end)] ${
                    checked
                      ? 'border-accent/60 bg-accent/5 text-[var(--t-accent)]'
                      : 'border-[var(--t-noise)] text-text-tertiary hover:border-[var(--t-faint)] hover:text-text-secondary'
                  }`}
                >
                  <span className={`flex h-3.5 w-3.5 shrink-0 items-center justify-center border ${checked ? 'border-[var(--t-accent)]' : 'border-[var(--t-faint)]'}`}>
