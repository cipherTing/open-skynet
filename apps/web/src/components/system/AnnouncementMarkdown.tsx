      ...
      <ReactMarkdown
        ...
        components={{
          a: ({ href, children }) => {
            ...
            if (href.startsWith('/')) {
              return (
                <Link href={href} className="text-[var(--t-accent)] underline underline-offset-2 hover:text-white">
                  {children}
                </Link>
              );
            }
            return (
              <a
                ...
                className="text-[var(--t-accent)] underline underline-offset-2 hover:text-white"
              >
                {children}
              </a>
            );
          ...
          blockquote: ({ children }) => (
            <blockquote className="my-2 border-l-2 border-[var(--t-accent)] pl-3 text-white/70">
              {children}
            </blockquote>
          ),
          pre: ({ children }) => (
            <pre className="my-2 overflow-x-auto rounded-none border border-[var(--t-noise)] bg-black px-3 py-2 text-xs text-white/70">
              {children}
            </pre>
          ),
          code: ({ children }) => (
            <code className="rounded-none border border-[var(--t-noise)] bg-[var(--t-panel)] px-1 py-0.5 font-mono text-[0.9em] text-white">
              {children}
            </code>
          ),
          table: ({ children }) => (
          ...
          th: ({ children }) => <th className="border border-[var(--t-noise)] px-2 py-1 font-bold">{children}</th>,
          td: ({ children }) => <td className="border border-[var(--t-noise)] px-2 py-1">{children}</td>,
        }}
      >
        {content}
      </ReactMarkdown>
