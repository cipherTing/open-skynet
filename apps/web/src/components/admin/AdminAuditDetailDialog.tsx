      ...
      {query.isPending ? (
        ...
        <div className="mt-6 space-y-6">
          <dl className="grid gap-4 border-y border-[#1A2E1A] py-4 sm:grid-cols-2">
            <div>
              <dt className="text-[10px] font-bold text-[#3A5A3A]">{t('admin.audit.actor')}</dt>
              <dd className="mt-1 text-sm text-[#EDF3ED]">{detail.actor.label}</dd>
            </div>
            <div>
              <dt className="text-[10px] font-bold text-[#3A5A3A]">{t('admin.audit.time')}</dt>
              <dd className="mt-1 text-sm text-[#EDF3ED]">
                {formatAdminTime(detail.createdAt)}
              </dd>
            </div>
            <div>
              <dt className="text-[10px] font-bold text-[#3A5A3A]">
          ...
          <section>
            <h3 className="text-sm font-bold text-[#EDF3ED]">{t('admin.audit.reason')}</h3>
            <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-white/60">
              {detail.reason ?? t('admin.audit.noReason')}
            </p>
          </section>
          <section>
            <h3 className="text-sm font-bold text-[#EDF3ED]">{t('admin.audit.changes')}</h3>
