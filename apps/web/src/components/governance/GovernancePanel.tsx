...
export function GovernancePanelContent() {
  ...
  return (
    <div className="skynet-auto-hide-scrollbar flex h-full min-h-0 flex-col overflow-y-auto overscroll-contain">
        ...
        <span className="flex flex-none items-center gap-1.5 font-mono text-[10px] uppercase tracking-[0.15em] text-[var(--t-faint)]">
          <span
            ...
            className={`h-1.5 w-1.5 ${
              statsQuery.isFetching ? 't-anim-blink bg-[var(--t-accent)]' : 'bg-[var(--t-faint)]'
            }`}
          />
      ...
      <section className="border-b border-[var(--t-noise)] px-4 py-4">
        <h2 className="text-sm font-bold text-white">{t('governance.panel.overview')}</h2>
        <p className="mt-1 text-[11px] leading-5 text-[var(--t-sub)]">
          ...
          <section className="border-b border-[var(--t-noise)] p-3">
            <div className="grid grid-cols-2 gap-px border border-[var(--t-noise)] bg-[var(--t-noise)]">
              ...
              <PanelStat
                ...
                valueClass="text-[var(--t-hazard)]/90"
              />
              <PanelStat
                icon={Clock}
...
export function GovernancePanel() {
  return (
    <aside className="hidden h-full min-h-0 w-[280px] shrink-0 flex-col border-l border-[var(--t-noise)] bg-black xl:flex">
...
function DistributionRow({
  ...
  return (
    <div className="flex items-center justify-between py-2">
      ...
      <span className="inline-block whitespace-nowrap font-mono text-sm tabular-nums text-white/80">
        {formatInteger(value)}
      </span>
    </div>
  );
