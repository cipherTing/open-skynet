...
Docker Hub 公开仓库固定为 `sundayting/skynet-api` 和 `sundayting/skynet-web`。`main` 的成功提交发布成 `dev-<完整 Git SHA>`；通过发布验收的 Git tag 同时更新对应 SemVer 镜像和 `latest`，其中 `latest` 始终指向最近一次通过验收的版本。
