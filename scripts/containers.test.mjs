...
test('production Compose template uses one release tag for API, indexes, and Web', () => {
  ...
  assert.match(production, /image: sundayting\/skynet-api:\$\{SKYNET_IMAGE_TAG:-0\.1\.0-rc2\}/u);
  assert.match(production, /image: sundayting\/skynet-web:\$\{SKYNET_IMAGE_TAG:-0\.1\.0-rc2\}/u);
  assert.equal(
    (production.match(/image: sundayting\/skynet-api:\$\{SKYNET_IMAGE_TAG:-0\.1\.0-rc2\}/gu) ?? [])
      .length,
    2,
  );
...
test('environment template exposes deployment ports, proxy boundaries, one image tag, and secrets', () => {
  ...
  assert.match(envExample, /^SKYNET_IMAGE_TAG=0\.1\.0-rc2$/mu);
  for (const name of [
    'WEB_PORT',
    'API_PORT',
    'MONGO_PORT',
