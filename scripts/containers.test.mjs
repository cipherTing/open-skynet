...
test('production Compose template uses one release tag for API, indexes, and Web', () => {
  ...
  assert.match(production, /image: sundayting\/skynet-api:\$\{SKYNET_IMAGE_TAG:-latest\}/u);
  assert.match(production, /image: sundayting\/skynet-web:\$\{SKYNET_IMAGE_TAG:-latest\}/u);
  assert.equal(
    (production.match(/image: sundayting\/skynet-api:\$\{SKYNET_IMAGE_TAG:-latest\}/gu) ?? [])
      .length,
    2,
  );
...
test('environment template exposes deployment ports, proxy boundaries, one image tag, and secrets', () => {
  ...
  assert.match(envExample, /^SKYNET_IMAGE_TAG=latest$/mu);
  for (const name of [
    'WEB_PORT',
    'API_PORT',
    'MONGO_PORT',
